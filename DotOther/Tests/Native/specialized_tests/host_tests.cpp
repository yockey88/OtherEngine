 ...
 protected:
  HostConfig config = {
    ...
    .exception_callback = [](const NString message) {
        DOTOTHER_LOG(DO_STR("C# Exception Caught : \n\t{}"sv), MessageLevel::DEBUG, message);
        FAIL(); },
    .log_callback = [](const NString message, MessageLevel level) { fmt::print("[MANAGED LOG HOOK] > {} [{}]\n"sv, message, level); },
    .invoke_native_method_hook = [](uint64_t object_handle, const NString method_name) {
        std::string mname = method_name;
        DOTOTHER_LOG(DO_STR("Invoking Native Method on object {:#08x}"sv) , MessageLevel::DEBUG , object_handle);
        DOTOTHER_LOG(DO_STR(" > Method Name: {}"sv) , MessageLevel::DEBUG, mname);
  ...
  virtual void SetUp() {
    ...
    DOTOTHER_LOG(DO_STR("Native Object Handle: {:#08x}"sv), MessageLevel::DEBUG, native_object->object_handle);
  }
...
TEST_F(HostTests, load_asm_and_call_functions) {
  ...
  native_object->proxy->object_handle(0xdeadbeef);
  DOTOTHER_LOG(DO_STR("Native Object Handle: {:#08x}"sv), MessageLevel::DEBUG, native_object->object_handle);
  ASSERT_EQ(native_object->object_handle, 0xdeadbeef);
  ...
  DOTOTHER_LOG(DO_STR("Assembly Context ID : {}"sv), MessageLevel::DEBUG, asm_ctx.context_id);
  ...
  DOTOTHER_LOG(DO_STR("Creating Instance Type: {}"sv), MessageLevel::DEBUG, type.FullName());
  HostedObject obj;
  ...
  DOTOTHER_LOG(DO_STR("Invoking Internal Call"sv), MessageLevel::DEBUG);
  ...
  DOTOTHER_LOG(DO_STR("Field Value: {}"sv), MessageLevel::DEBUG, number);

  expected = 69.f;
  ASSERT_NO_FATAL_FAILURE(obj.SetProperty("MyNum", expected));
  ASSERT_NO_FATAL_FAILURE(number = obj.GetProperty<float>("MyNum"sv));
  ASSERT_EQ(number, expected);
  DOTOTHER_LOG(DO_STR("Property Value: {}"sv), MessageLevel::DEBUG, number);
