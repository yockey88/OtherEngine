...
class BufferTests : public other::OtherTest {
 public:
  ...
  void SetUp() override {
    ...
    ASSERT_EQ(buffer.Size(), 0u) << "Buffer size is not initially zero!";
    ASSERT_EQ(buffer.Capacity(), 0u) << "Buffer capacity is not initially zero!";
    ASSERT_EQ(buffer.NumElements(), 0u) << "Buffer number of elements is not 0 at the beginning of test!";
  }
...
TEST_F(BufferTests, idx_from_buffer_data) {
  uint32_t val = 33u;
  uint64_t val2 = 44u;
  ...
  EXPECT_NO_FATAL_FAILURE(val_idx = buffer.BufferData(val));
  EXPECT_NO_FATAL_FAILURE(val2_idx = buffer.BufferData(val2));
  EXPECT_NO_FATAL_FAILURE(mat4_idx = buffer.BufferData(mat4));
  EXPECT_NO_FATAL_FAILURE(vec2_idx = buffer.BufferData(vec2));
...
void foo(const Buffer& b) {
  std::cout << b.DumpBuffer() << "\n";

  std::cout << "Buffer size = " << b.Size() << " | Num Elements = " << b.NumElements() << "\n";
  for (size_t i = 0; i < b.NumElements(); ++i) {
    std::cout << "Element " << i << "\n ";
    // access each element
    const glm::mat4& mat = b.At<glm::mat4>(i);
    std::cout << fmt::format("{}\n"sv, mat);
  }
};
...
TEST_F(BufferTests, pass_by_const_ref) {
  ...
  buffer.BufferData(glm::mat4(1.f));
  ASSERT_NO_FATAL_FAILURE(foo(buffer));
