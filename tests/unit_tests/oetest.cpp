...
namespace other {
  ...
  Scope<Engine> OtherTest::mock_engine = nullptr;
    ...
    for (auto& module : modules) {
      ...
      switch (module.second.module->GetLanguageType()) {
        case LanguageModuleType::CS_MODULE:
          cs_good = scripts.size() == cs;
          break;
        case LanguageModuleType::LUA_MODULE:
          lua_good = scripts.size() == lua;
          break;
        case LanguageModuleType::PYTHON_MODULE:
          python_good = scripts.size() == python;
          break;
        default:
          break;
      }
    }
    return cs_good && lua_good && python_good;
  }
