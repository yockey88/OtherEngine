...
class OtherCliPipeline(Pipeline):
    ...
    def run(self):
        ...
        if env.get_settings().create_project:
            ...
            self._process_error(self._try_create_project(
            ), " > project creation successful", " !> project creation failed!")
            return 0

        if env.get_settings().generate_files:
            self._process_error(self._gen_files(
            ), " > file generation successful", " !> file generation failed!")
        if env.get_settings().generate_projects or env.get_settings().generate_files:
            self._process_error(self._gen_projects(
            ), " > project generation successful", " !> project generation failed!")

        self._process_error(self._try_build())
        if env.should_test():
            self._process_error(
                self._try_test(), " > testing successful", " !> testing failed!")
            sys.exit(0)
