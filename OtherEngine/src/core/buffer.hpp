...
namespace other {
  ...
  class Buffer {
   public:
    ...
    template <typename T>
    size_t BufferData(const T& value) {
    ...
    template <typename T>
    size_t ValidateAtCall(size_t idx) const {
      OE_ASSERT(idx < element_sizes.size(), "Attempting to retrieve invalid index! expected {} > {} num elements", idx, element_sizes.size());
