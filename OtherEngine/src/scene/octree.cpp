...
namespace {
  ...
  void Octant::AddEntity(Entity* entity , const glm::vec3& position) {
    if (!Contains(position)) {
      ...
      OE_ASSERT(Contains(position) , "Octant {} does not contain point {}" , tree_index , position);
    }
  ...
  void Octant::Serialize(writer& w , bool print_children) const {
    ...
    w << fmtstr("(@Location({:>03b}) @Partition({}) @Dimensions{}  @Center{})\n", 
                partition_index , partition_location , bbox.extent - origin , origin);
    std::stringstream ss;
    if (entities.empty()) {
      w << "(@Entities = 0)\n"s;
    } else {
  ...
  Octree::Octree()
      : global_position(glm::zero<glm::vec3>()) , depth(0), num_octants(NumOctantsAtDepth(0)) {
    Initialize(glm::zero<glm::vec3>());
  }

  Octree::Octree(const glm::vec3& space_dimensions , const glm::vec3& origin , size_t resolution) 
      : global_position(origin) , depth(resolution) , num_octants(NumOctantsAtDepth(resolution)) {
  ...
  void Octree::MoveOriginTo(const glm::vec3& origin) {
    ...
    // space->bbox = BoundingBox(global_position - dimensions / 2.f , global_position + dimensions / 2.f);
  }
  ...
  Octant& Octree::GetContainingOctant(const glm::vec3& point) {
    ...
    OE_DEBUG("POINT : {} , GLOBAL : {}" , point , global_position);
