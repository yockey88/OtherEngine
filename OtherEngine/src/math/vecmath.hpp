...
namespace other {
  ...
  template <typename T>
  concept glm_vec_t =
    std::is_same_v<glm::vec4, T> || std::is_same_v<glm::vec3, T> ||
    std::is_same_v<glm::vec2, T>;

  template <typename T>
  concept glm_mat_t =
    std::is_same_v<glm::mat4, T> || std::is_same_v<glm::mat3, T> ||
  ...
  template <typename T, typename U>
  concept product_defined = requires(T t, U u) { t* u; };

  template <typename T, typename U>
  concept measurable = subtractable<T, U> && requires(T t, U u) {
    { std::abs(t - u) } -> std::convertible_to<float>;
  };

  template <typename T, typename U>
  concept vec_measureable = subtractable<T, U> && requires(T t, U u) {
  ...
  template <typename T>
  concept epsilon_defined = requires(T t) { std::numeric_limits<T>::epsilon(); };

  template <typename T>
  concept epsilon_comparable =
    epsilon_defined<T> && zero_initializable<T> &&
    requires(T t) { t < std::numeric_limits<T>::epsilon(); };
  ...
  template <typename T, typename U>
  concept epsilon_comparable_with =
    comparable<T, U> && multi_epsilon_comparable<T, U>;

  template <typename T>
  concept defined_infinity =
