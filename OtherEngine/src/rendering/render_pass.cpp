...
namespace other {
  ...
  RenderPass::RenderPass(RenderPassSpec spec)
    ...
    DefineInput({ "goe_specular", SAMPLER2D, sizeof(uint32_t) });
    DefineInput({ "goe_shadow_map", SAMPLER2D, sizeof(uint32_t) });
    DefineInput({ "goe_depth_map", SAMPLER2D, sizeof(uint32_t) });
