...
namespace other {
  ...
  namespace ui {
    ...
    bool DragInt8(const char* label, int8_t* v, float v_speed, int8_t v_min, int8_t v_max, const char* format, ImGuiSliderFlags flags) {
      bool changed = ImGui::DragScalar(label, ImGuiDataType_S8, v, v_speed, &v_min, &v_max, format, flags);
      DrawItemActivityOutline();
      return changed;
    }

    bool DragUInt8(const char* label, uint8_t* v, float v_speed, uint8_t v_min, uint8_t v_max, const char* format, ImGuiSliderFlags flags) {
      bool changed = ImGui::DragScalar(label, ImGuiDataType_U8, v, v_speed, &v_min, &v_max, format, flags);
      DrawItemActivityOutline();
      return changed;
    }

    bool DragInt16(const char* label, int16_t* v, float v_speed, int16_t v_min, int16_t v_max, const char* format, ImGuiSliderFlags flags) {
      bool changed = ImGui::DragScalar(label, ImGuiDataType_S16, v, v_speed, &v_min, &v_max, format, flags);
      DrawItemActivityOutline();
      return changed;
    }

    bool DragUInt16(const char* label, uint16_t* v, float v_speed, uint16_t v_min, uint16_t v_max, const char* format, ImGuiSliderFlags flags) {
      bool changed = ImGui::DragScalar(label, ImGuiDataType_U16, v, v_speed, &v_min, &v_max, format, flags);
      DrawItemActivityOutline();
      return changed;
    }

    bool DragInt32(const char* label, int32_t* v, float v_speed, int32_t v_min, int32_t v_max, const char* format, ImGuiSliderFlags flags) {
      bool changed = ImGui::DragScalar(label, ImGuiDataType_S32, v, v_speed, &v_min, &v_max, format, flags);
      DrawItemActivityOutline();
      return changed;
    }

    bool DragUInt32(const char* label, uint32_t* v, float v_speed, uint32_t v_min, uint32_t v_max, const char* format, ImGuiSliderFlags flags) {
      bool changed = ImGui::DragScalar(label, ImGuiDataType_U32, v, v_speed, &v_min, &v_max, format, flags);
      DrawItemActivityOutline();
      return changed;
    }

    bool DragInt64(const char* label, int64_t* v, float v_speed, int64_t v_min, int64_t v_max, const char* format, ImGuiSliderFlags flags) {
      bool changed = ImGui::DragScalar(label, ImGuiDataType_S64, v, v_speed, &v_min, &v_max, format, flags);
      DrawItemActivityOutline();
      return changed;
    }

    bool DragUInt64(const char* label, uint64_t* v, float v_speed, uint64_t v_min, uint64_t v_max, const char* format, ImGuiSliderFlags flags) {
      bool changed = ImGui::DragScalar(label, ImGuiDataType_U64, v, v_speed, &v_min, &v_max, format, flags);
      DrawItemActivityOutline();
      return changed;
    }
    ...
    void DrawButtonImage(const Ref<Texture2D>& image_normal, const Ref<Texture2D>& image_hovered, const Ref<Texture2D>& image_pressed, ImU32 tint_normal, ImU32 tint_hovered, ImU32 tint_pressed, ImVec2 rect_min, ImVec2 rect_max) {
    ...
    void DrawButtonImage(const Ref<Texture2D>& image, ImU32 tint_normal, ImU32 tint_hovered, ImU32 tint_pressed, ImVec2 rect_min, ImVec2 rect_max) {
      DrawButtonImage(image, image, image, tint_normal, tint_hovered, tint_pressed, rect_min, rect_max);
    }

    bool TreeNodeWithIcon(const Ref<Texture2D>& icon, ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end, ImColor icon_tint) {
      ImGuiWindow* window = ImGui::GetCurrentWindow();
      if (window->SkipItems) {
        return false;
      }
      ...
      if (!item_add) {
        ...
        IMGUI_TEST_ENGINE_ITEM_INFO(last_item.ID, label, last_item.StatusFlags | (is_leaf ? 0 : ImGuiItemStatusFlags_Openable) | (is_open ? ImGuiStatusFlags_Opened : 0));
        return is_open;
      }
      ...
      if (display_frame) {
        const ImU32 bg_col = ImGui::GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : (hovered && !selected && !held && !pressed && !toggled) ? ImGuiCol_HeaderHovered :
                                                                                                                                                      ImGuiCol_Header);
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet) {
          ...
          ImGui::RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offs_x + padding.x, text_pos.y), arrow_col, is_open ? ImGuiDir_Down : ImGuiDir_Right, 1.f);
        } else {
