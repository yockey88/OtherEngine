...
namespace other {
  ...
  namespace {
    ...
    uint32_t EnableGlSettings(const other::ConfigTable& config) {
      ...
      auto depth_test = config.GetVal<bool>(kRendererSection, kDepthTestValue, false).value_or(true);
      auto stencil_test = config.GetVal<bool>(kRendererSection, kStencilTestValue, false).value_or(true);
      auto cull_face = config.GetVal<bool>(kRendererSection, kCullFaceValue, false).value_or(false);
      auto multisample = config.GetVal<bool>(kRendererSection, kMultisampleValue, false).value_or(true);
      auto blend = config.GetVal<bool>(kRendererSection, kBlendValue, false).value_or(false);
    ...
    void EnableSdlGlSettings(const other::ConfigTable& config) {
      int32_t gl_major = config.GetVal<uint32_t>(kRendererSection, kMajorValue, false).value_or(4);
      int32_t gl_minor = config.GetVal<uint32_t>(kRendererSection, kMinorValue, false).value_or(6);
  ...
  WindowConfig Window::ConfigureWindow(const other::ConfigTable& config) {
    ...
    const auto xpos = config.GetVal<int>(kWindowSection, kXposValue, false);
    const auto ypos = config.GetVal<int>(kWindowSection, kYposValue, false);

    cfg.pos.x = xpos.value_or(SDL_WINDOWPOS_CENTERED);
    cfg.pos.y = ypos.value_or(SDL_WINDOWPOS_CENTERED);

    cfg.centered = config.GetVal<bool>(kWindowSection, kCenteredValue, false).value_or(true);
    if (cfg.centered) {
