...
namespace other {
  ...
  CBActionResult ContentBrowserItem::Render() {
    ...
    } else if (ImGui::ItemHoverable(ImRect(top_l, bot_r), ImGui::GetID(&file_handle), ImDrawFlags_RoundCornersBottom)) {
      auto* draw_list = ImGui::GetWindowDrawList();
    ...
    ui::DrawButtonImage(icon, IM_COL32(255, 255, 255, 255), IM_COL32(255, 255, 255, 255), IM_COL32(255, 255, 255, 255), ui::RectExpanded(ui::GetItemRect(), -6.f, -6.f));
    ...
    if (type == Type::DIRECTORY) {
      ...
      if (display_asset_type) {
        ...
        ImGui::Spring();
        {
          switch (type) {
            case Type::DIRECTORY: {
              ScopedColor text_col(ImGuiCol_Text, ui::theme::muted);
              ImGui::Text("Directory");
            } break;
            case Type::FILE: {
              Ref<FileHandle> file = Filesystem::GetFile(file_handle);
              if (file == nullptr) {
                ImGui::Text("Invalid File");
                break;
              }

              ImGui::Text("%s", fmtstr("File [{}]", file->GetAssetType()).c_str());
              break;
            }
            default: {
              ScopedColor text_col(ImGuiCol_Text, ui::theme::red);
              ImGui::Text("Invalid Content Browser Item Type (not a file or a directory)");
            } break;
          }
        }
        ImGui::EndHorizontal();
      ...
      if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_SourceAllowNullID)) {
        ...
        std::string_view tag_name = VirtualDriveFromExtension(Path{ filename }.extension().string());

        ImGui::SetDragDropPayload(fmtstr("asset-payload:{}", tag_name).c_str(), &file_handle, sizeof(AssetHandle));

        result.Set(CBAction::DRAGGING, true);
    ...
    if (selected || ImGui::IsItemHovered()) {
      ...
      if (selected) {
        ...
        drawlist->AddRect(rect.Min, rect.Max, mouse_down ? ImColor(col_transition) : ImColor(ui::theme::selection), 6.f, type == Type::DIRECTORY ? 0 : ImDrawFlags_RoundCornersBottom, 1.f);
    ...
    if (ImGui::IsItemHovered()) {
      ...
      if (ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left) && !renaming) {
        ...
        bool skip_bc_dragging = result.Has(CBAction::DRAGGING) && selected;
  ...
  bool ContentBrowserItem::Move(const Path& new_path) {
    return false;
  }
  ...
  void ContentBrowserItem::OnRenamed(const std::string& new_name) {
    filename = new_name;
  }

  std::string ContentBrowserItem::OverrideDisplayName(const std::string& new_name) {
    return new_name;
  };
  ...
  UUID ContentBrowserItem::GetHandle() const {
    return file_handle;
  }
  ...
  CBDirectory::CBDirectory(const Ref<Directory>& directory)
      : ContentBrowserItem(Type::DIRECTORY, directory->handle, directory->ProjectRelativePath().stem().string(), EditorImages::folder_icon),
        directory(directory) {
    for (auto& [id, child] : directory->children) {
      children.push_back(NewRef<CBDirectory>(child));
    }
  }
  ...
  CBItem::CBItem(const Ref<FileHandle>& asset_data, const Ref<Texture2D>& texture)
      : ContentBrowserItem(Type::FILE, asset_data->handle, asset_data->ProjectRelativePath().filename().string(), texture), asset_data(asset_data) {}
