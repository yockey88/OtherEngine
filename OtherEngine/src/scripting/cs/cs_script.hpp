...
namespace other {
  ...
  class CsScript : public ScriptModule {
   public:
    CsScript(CsModule* host, const std::string& module_name, ref<Assembly> assembly)
        : ScriptModule(LanguageModuleType::CS_MODULE, module_name), host(host), assembly(assembly) {}
