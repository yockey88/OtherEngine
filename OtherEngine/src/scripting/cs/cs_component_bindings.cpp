...
namespace other {
  namespace {
    ...
    static void RegisterComponent(ref<Assembly> assembly, auto... getters_and_setters) {
      const std::string_view component_type = TypeData<T>().Name();
    ...
    static void RegisterComponent(std::function<void(Entity*)>&& add_fn, ref<Assembly> assembly) {
      const std::string& component_type = TypeData<T>().Name();
      std::string name = fmtstr("Other.{}", component_type);
    ...
    template <typename T>
    void RegisterCompatibleProperty(const std::string_view klass_name, const std::string_view prop_name, ref<Assembly> assembly, CompatibleGetter<T> getter = nullptr, CompatibleSetter<T> setter = nullptr) {
    ...
    template <typename T>
    void RegisterProperty(const std::string_view klass_name, const std::string_view prop_name, ref<Assembly> assembly, Accessor<T> getter = nullptr, Accessor<T> setter = nullptr) {
      const std::string name = fmtstr("Other.{}", klass_name);
      Type& type = assembly->GetType(name);
      if (type.handle == -1) {
        OE_ASSERT(false, "NO C# type found for {}", name);
    ...
    template <typename T, typename C, T C::*member>
    void RegisterComponentMember(const std::string_view klass_name, const std::string_view member_name, ref<Assembly> assembly, MemberAccessor<T, C, member> custom_getter = nullptr, MemberAccessor<T, C, member> custom_setter = nullptr) {
      const std::string name = fmtstr("Other.{}", klass_name);
      Type& type = assembly->GetType(name);
      if (type.handle == -1) {
        OE_ASSERT(false, "NO C# type found for {}", name);
  ...
  namespace cs_script_bindings {
    ...
    void RegisterNativeComponents(ref<Assembly> assembly) {
      ...
      RegisterComponentMember<glm::vec3, Transform, &Transform::erotation>("Transform", "Rotation", assembly, nullptr, [](Entity* entity, glm::vec3* value) {
        OE_ASSERT(entity != nullptr, "Entity is null!");
