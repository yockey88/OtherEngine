...
namespace other {
  ...
  Ref<AstStmt> ShaderParser::ParseInOutBlock(const Token& type, const Token& tag) {
    ...
    if (Match({ OPEN_BRACKET })) {
      ...
      Consume(CLOSE_BRACKET, fmtstr("Unterminated array {} block found! Cant not define length of in/out block arrays! found {}", type.value, Peek().value));
    }
  ...
  bool ShaderParser::MatchLiterals(bool advance) {
    return Match({ INT_LIT, FLOAT_LIT });
  }
