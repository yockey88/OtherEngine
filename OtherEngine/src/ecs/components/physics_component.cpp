...
namespace other {
  ...
  RigidBodySnapshotter::RigidBodySnapshotter() {
  ...
  ColliderSnapshotter::ColliderSnapshotter() {
    AddField<uint32_t, 0>(&Collider::shape_idx);
    AddField<glm::vec3, 1>(&Collider::collider_scale);
  }

  void PhysicsObjectSnapshotter::Write(ByteBuffer& stream, const PhysicsObject& object) {
  }

  PhysicsObject PhysicsObjectSnapshotter::Read(ByteBuffer& stream, size_t buffer_offset) {
