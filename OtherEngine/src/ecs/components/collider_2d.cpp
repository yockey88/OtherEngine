...
namespace other {
  ...
  void Collider2DSerializer::Deserialize(Entity* entity, const ConfigTable& scene_table, Ref<Scene>& scene) const {
    ...
    collider.density = scene_table.GetVal<float>(key_value, kDensityValue, false).value_or(1.f);
    collider.friction = scene_table.GetVal<float>(key_value, kFrictionValue, false).value_or(1.f);
  }
