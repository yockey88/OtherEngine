...
namespace other {
  ...
  void RigidBody2DSerializer::Deserialize(Entity* entity, const ConfigTable& scene_table, Ref<Scene>& scene) const {
    ...
    rigid_body.gravity_scale = scene_table.GetVal<float>(key_value, kGravityScaleValue, false).value_or(1.f);
    rigid_body.fixed_rotation = scene_table.GetVal<bool>(key_value, kFixedRotationValue, false).value_or(false);
    rigid_body.bullet = scene_table.GetVal<bool>(key_value, kBulletValue, false).value_or(false);
  }
