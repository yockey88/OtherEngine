...
struct SpanInfo {
  ByteRange lineByteRange; // Begin/end range of the text buffer for this line,
                           // as always end is one beyond the end.
  std::vector<LineCharInfo> lineCodePoints; // Codepoints
  long bufferLineNumber = 0; // Line in the original buffer, not the screen line
  float yOffsetPx = 0.0f; // Position in the buffer in pixels, if the screen was
                          // as big as the buffer.
  NVec2f lineTextSizePx = NVec2f(0.0f); // Pixel size of the text
  int spanLineIndex =
      0; // The index of this line in spans; might be more than buffer index
  ...
  bool BufferCursorInside(GlyphIterator offset) const {
    return offset.Index() >= lineByteRange.first &&
           offset.Index() < lineByteRange.second;
  }
};
...
struct ToolTipMessage : public ZepMessage {
  ToolTipMessage(ZepBuffer *pBuff, const NVec2f &pos,
                 const GlyphIterator &loc = GlyphIterator())
...
private:
  ...
  void GetCharPointer(GlyphIterator loc, const uint8_t *&pBegin,
                      const uint8_t *&pEnd, SpecialChar &specialChar);
  ...
  void PlaceToolTip(const NVec2f &pos, ToolTipPos location, uint32_t lineGap,
...
private:
  ...
  std::shared_ptr<Region>
      m_bufferRegion; // region of the display we are showing on.
  ...
  std::shared_ptr<Region>
      m_indicatorRegion; // Indicators (between numbers and text)
  std::shared_ptr<Region> m_vScrollRegion; // Vertical scroller
  std::shared_ptr<Region>
      m_expandingEditRegion; // Region containing the text sub-box
  Airline m_airline;
  ...
  // This will indeed stop lines wrapping though!  You just can't move to the
  // far right and have the text scroll; which isn't a big deal, but a work
  // item.
  ...
  // Cursor
  GlyphIterator m_bufferCursor; // Location in buffer coordinates.  Each window
                                // has a different buffer cursor
  long m_lastCursorColumn =
      0; // The last cursor column (could be removed and recalculated)
  NVec2f m_mousePos;             // Current mouse location
  GlyphIterator m_mouseIterator; // Current iterator for the mouse cursor

  // Visual stuff
  std::vector<std::string>
      m_statusLines; // Status information, shown under the buffer

  // Setup of displayed lines
  std::vector<SpanInfo *>
      m_windowLines; // Information about the currently displayed lines
  float m_textOffsetPx = 0.0f; // The Scroll position within the text
  NVec2f m_textSizePx; // The calculated size of the buffer text, containing
                       // just the text
  NVec2i m_visibleLineIndices = {0,
                                 0}; // Index of the line spans that are visible
  ...
  GlyphIterator m_mouseBufferLocation; // The character in the buffer the tip
                                       // pos is over, or -1
  ...
  std::map<NVec2f, std::shared_ptr<RangeMarker>>
      m_toolTips; // All tooltips for a given position, currently only 1 at a
                  // time
