...
bool HeightField::init(int nbGridColumns, int nbGridRows, const void* heightFieldData, HeightDataType dataType, std::vector<Message>& messages, decimal integerHeightScale) {
  bool isValid = true;
...
void HeightField::computeOverlappingTriangles(const AABB& aabb, Array<Vector3>& triangleVertices, Array<Vector3>& triangleVerticesNormals, Array<uint32>& shapeIds, const Vector3& scale) const {
...
bool HeightField::raycast(const Ray& ray, RaycastInfo& raycastInfo, Collider* collider, TriangleRaycastSide testSide, MemoryAllocator& allocator) const {
...
bool HeightField::raycastTriangle(const Ray& ray, const Vector3& p1, const Vector3& p2, const Vector3& p3, uint32 shapeId, Collider* collider, RaycastInfo& raycastInfo, decimal& smallestHitFraction, TriangleRaycastSide testSide, MemoryAllocator& allocator) const {
  // Generate the first triangle for the current grid rectangle
