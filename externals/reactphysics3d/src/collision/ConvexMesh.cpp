...
bool ConvexMesh::init(const PolygonVertexArray& polygonVertexArray, std::vector<Message>& errors) {
  ...
  mHalfEdgeStructure.reserve(polygonVertexArray.getNbFaces(), polygonVertexArray.getNbVertices(), (polygonVertexArray.getNbVertices() + polygonVertexArray.getNbFaces() - 2) * 2);
...
bool ConvexMesh::createHalfEdgeStructure(const PolygonVertexArray& polygonVertexArray, std::vector<Message>& errors) {
  ...
  for (uint32 f = 0; f < nbFaces; f++) {
    ...
    if (faceVertices.size() < 3) {
      ...
      RP3D_LOG("PhysicsCommon", Logger::Level::Error, Logger::Category::PhysicCommon, "Error when creating a ConvexMesh: Mesh has a face with less than three vertices.", __FILE__, __LINE__);
    }
  ...
  if (2 + nbEdges - polygonVertexArray.getNbFaces() != polygonVertexArray.getNbVertices()) {
    RP3D_LOG("PhysicsCommon", Logger::Level::Error, Logger::Category::PhysicCommon, "Error when creating a ConvexMesh: input PolygonVertexArray is not valid. Mesh with duplicated vertices is not supported.", __FILE__, __LINE__);
...
Vector3 ConvexMesh::computeFaceNormal(uint32 faceIndex) const {
  ...
  for (uint32 i = face.faceVertices.size() - 1, j = 0; j < face.faceVertices.size(); i = j, j++) {
    ...
    normal += Vector3((v1.y - v2.y) * (v1.z + v2.z), (v1.z - v2.z) * (v1.x + v2.x), (v1.x - v2.x) * (v1.y + v2.y));
  }
